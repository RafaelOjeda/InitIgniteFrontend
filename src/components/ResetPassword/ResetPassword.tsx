import AuthServiceInstance, {Email} from "../GeneralComponents/AuthService.js";
...
const ResetPassword = () => {
	...
	const handleReset = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
		...
		try {
			...
			setIsEmailSent(true); // Update state after successful reset
		} catch (error) {
			console.error('Error resetting password:', error);
			setMissingInfoMessage('Failed to send reset email. Please try again.');
		}
	};
		...
		<form onSubmit={handleReset}>
			...
			>
				...
				<Box sx={{
					...
					<button className="btn btn-success" type="submit" style={{ marginTop: '0.75rem', width: '275px' }} >
					  Reset Password
					</button>
					<button className="btn btn-info" type="button"
							onClick={() => {
								navigate('/login');
								AuthServiceInstance.hasPermission("default", "post.view")
							}}
							style={
								{
									marginTop: '1rem',
